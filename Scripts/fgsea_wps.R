...
count_data <- read.delim("processed_wps.txt", header = TRUE, row.names = 1, sep = "\t")
...
#write.table(results, file = "limma_wps_logFC_P.txt", sep = "\t")
...
significant_pathways <- subset(GSEAres, pval < 0.05)
# View the significant pathways
print(significant_pathways)
GSEAres_sorted <- GSEAres[order(GSEAres$pval), ]
head(GSEAres_sorted)
# Create a volcano plot
#volcano_plot <- ggplot(results, aes(x = logFC, y = -log10(p_value))) + geom_point(alpha = 0.5) + geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "red") + labs(x = "Log2 Fold Change", y = "-log10(P-value)", title = "WPS Plot")
#volcano_plot <- volcano_plot + geom_text(aes(label = rownames(results)), , vjust = -0.5, size = 1.5)
#ggsave("new_wps_plot.png", plot = volcano_plot, width = 6, height = 4, units = "in", dpi = 300)
