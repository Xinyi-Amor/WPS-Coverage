...
write.table(results, file = "wps_logFC_P.txt", sep = "\t")
...
volcano_plot <- ggplot(results, aes(x = logFC, y = -log10(p_value))) +
  ...
  labs(x = "Log2 Fold Change", y = "-log10(P_value)", title = "WPS Plot")
volcano_plot <- volcano_plot + geom_text(aes(label = results$gene_name), vjust = -0.5, size = 1.5)
ggsave("wps.png", plot = volcano_plot, width = 6, height = 4, units = "in", dpi = 300)
