...
volcano_plot <- ggplot(results, aes(x = logFC, y = -log10(p_value))) + geom_point(alpha = 0.5) + geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "red") + labs(x = "Log2 Fold Change", y = "-log10(P-value)", title = "Volcano Plot")
volcano_plot <- volcano_plot + geom_text(aes(label = rownames(results)), , vjust = -0.5, size = 1.5)
ggsave("test_plot.png", plot = volcano_plot, width = 6, height = 4, units = "in", dpi = 300)
